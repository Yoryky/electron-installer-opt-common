...
class ElectronInstaller {
   ...
   */
  get stagingAppDir () {
    return path.join(this.stagingDir, this.baseAppDir, this.appIdentifier)
  }
  ...
  async copyHicolorIcons () {
    return Promise.all(_.map(this.options.icon, (iconSrc, resolution) => {
      ...
      const iconFile = path.join(this.stagingDir, 'usr', 'share', 'icons', 'hicolor', resolution, 'apps', `${iconName}.${iconExt}`)
  ...
  async copyPixmapIcon () {
    const iconFile = path.join(this.stagingDir, 'usr', this.pixmapIconPath)
  ...
  async createBinarySymlink () {
    const binSrc = path.join('../../opt', this.appIdentifier, this.options.bin)
    const binDest = path.join(this.stagingDir, 'usr', 'bin', this.appIdentifier)
  ...
  async createCopyright () {
    const copyrightFile = path.join(this.stagingDir, 'usr', 'share', 'doc', this.appIdentifier, 'copyright')
  ...
  async createDesktopFile () {
    ...
    const baseDir = path.join(this.stagingDir, 'usr', 'share', 'applications')
